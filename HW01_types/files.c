...
+ 1. Находим Central directory file header(cdfh), 0x504b0102, от его начала 29-30 байты - значение длинны имени файла.
...
long get_cdfh_filename_offset(FILE *fd, long offset_filelen){
        fseek(fd, offset_filelen, SEEK_SET);
...
int main(int argc, char **argv){
        ...
        if (offset != -1) {
                ...
                for(int i=0; i != 2; ++i){
                        ...
                        long offset_filelen = new_offset_filelen;
                        printf("\n\n");
                        printf("FILELEN must be at the offset %ld\n", new_offset_filelen);
                        long offset_filename = get_cdfh_filename_offset(fd, offset_filelen);
                        ...
                        get_cdfh_filelen_offset(fd, offset_filelen);
                        printf("\n\n");
                }
        }
        else {
