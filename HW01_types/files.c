...
---- Для начала цикла есть:
        a. Смещение имени первого файла.
        b. Смещение длинны имени первого файла.
        c. Длинна имени первого файла.
        d. Эти значения надо сохранить в переменные внутри цикла. 
        e. В конце итеррации цикла эти смещения надо как-то обновлять...мб можно их обновить для функций из которых ранее эти смещения и получили?
        f. Цикл должен записывать имя переменной как в функции get_filename_from_zip, но только в массив или структуру? Понятно, что и то и другое должно быть динамическим...
        g. Надо будет читать и выводить по элементу массива (или структуры?) в main, пока она не закончится...
*/
